...
const index = () => {
  ...
  useEffect(()=> {
    const userBackEndID = localStorage.getItem("Crypto_BackEnd");
  ...
  const buyMemberShip = async(memberType, price)=>{
    ...
    const wallet = new ethers.Wallet(`0x${networks?.praviteKey}`, 
    provider);
    const ammountToSend = ethers.parseUnits(price.toString(), "ether");

    const transaction = {
      // Please Add Transaction Account Key
      to: "0x00912Bf03a1d1768C8c256649a805089b672Ac31",
      value: ammountToSend,
    };
